      ...
      <div className="max-w-3xl mx-auto space-y-12 mt-6">
        ...
        <div className="space-y-8 bg-secondary p-8 rounded-[32px] border border-theme">
          <div>
            ...
            <input
              type="number"
              className="w-full bg-primary border-cyan-500/30 p-5 rounded-2xl text-cyan-400 font-black text-2xl outline-none focus:border-cyan-500 transition"
              value={baseBudget || ''}
              onChange={e => setBaseBudget(parseInt(e.target.value) || 0)}
            />
            <p className="text-[10px] text-secondary mt-2 ml-1">Enter the budget for base system development. Add-ons are selected below.</p>
          </div>

          {availableAddons.length > 0 && (
            <div className="space-y-3">
              <label className="text-[10px] font-black uppercase tracking-widest text-secondary block ml-1">Select Custom Add-ons</label>
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <div className="flex justify-between items-center mb-1">
                          <h4 className={`text-sm font-bold ${isSelected ? 'text-cyan-400' : 'text-primary'}`}>{addon.name}</h4>
          ...
          <div className="space-y-4 pt-6 border-t border-theme">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <input type="text" placeholder="John Doe" value={name} onChange={e => setName(e.target.value)} className="w-full bg-card border border-theme p-4 rounded-xl text-sm focus:border-cyan-500 outline-none text-primary font-bold hover:border-theme" required />
                <p className="text-[9px] text-secondary ml-1">Please enter your legal name as it appears on official files.</p>
              </div>
              <div className="space-y-1">
                <label className="text-[10px] text-secondary font-bold ml-1 uppercase">Email Address</label>
                <input type="email" placeholder="john.doe@example.com" value={email} onChange={e => setEmail(e.target.value)} className="w-full bg-card border border-theme p-4 rounded-xl text-sm focus:border-cyan-500 outline-none text-primary font-bold hover:border-theme" required />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <input type="tel" placeholder="+234..." value={whatsapp} onChange={e => setWhatsapp(e.target.value)} className="w-full bg-card border border-theme p-4 rounded-xl text-sm focus:border-cyan-500 outline-none text-primary font-bold hover:border-theme" required />
            ...
            <div className="space-y-1">
              <label className="text-[10px] text-secondary font-bold ml-1 uppercase">Project Title / Objective</label>
              <input type="text" placeholder="E.g., Real Estate Management Portal" value={topic} onChange={e => setTopic(e.target.value)} className="w-full bg-card border border-theme p-4 rounded-xl text-sm focus:border-cyan-500 outline-none text-primary font-bold hover:border-theme" required />
            </div>

            <div className="space-y-1">
              <label className="text-[10px] text-secondary font-bold ml-1 uppercase">System Instructions & Features</label>
              <textarea
                placeholder="List core features, tech stack requirements, user roles, API integrations..."
                className="w-full bg-card border border-theme p-4 rounded-xl text-sm focus:border-cyan-500 outline-none resize-none h-32 text-primary font-medium hover:border-theme"
                value={instructions}
            ...
            <div className="space-y-1">
              <label className="text-[10px] text-secondary font-bold ml-1 uppercase">Database Schema or Wireframes</label>
              <label className="border-2 border-dashed border-theme hover:border-cyan-500/50 bg-card rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer transition">
          ...
          <div className="space-y-4 pt-6 border-t border-theme">
            ...
            <label className="flex items-start gap-3 cursor-pointer">
              <input
                ...
                className="mt-1 w-4 h-4 accent-cyan-500 bg-primary border-theme rounded"
              />
          ...
          <div className="bg-cyan-500/10 p-6 rounded-2xl border border-cyan-500/20 text-center">
            ...
            <p className="text-[9px] uppercase font-black text-secondary mt-2 tracking-widest">Calculated Development Quote</p>
          </div>
