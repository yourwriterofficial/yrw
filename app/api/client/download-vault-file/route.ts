...
export async function POST(request: Request) {
  try {
    ...
    const { data: order, error: orderError } = await admin
      .from('orders')
      .select('order_id, client_id, email, payment_structure_type, payment_milestones, sixty_percent_paid, forty_percent_paid, topic')
    ...
    // 3. Authorize: the order must belong to this user (by id or email) or they must be an admin
