...
export default function LoggedInDevOrderPage() {
  ...
  const [baseBudget, setBaseBudget] = useState<number>(100000);
  ...
  const submitOrder = async () => {
    if (!topic || !deadline) {
      showToast('Please fill out all mandatory fields (Project Title, Deadline).', 'error');
      return;
    }
    if (!acceptTerms) {
    ...
    if (baseBudget < 20000) {
      showToast('Software projects require a minimum base budget of ₦20,000.', 'error');
      return;
    }
      ...
      <div className="max-w-3xl mx-auto space-y-12 mt-6">
        ...
        <div className="space-y-8 bg-secondary border border-theme p-8 rounded-[32px]">
          <div>
            ...
            <input
              type="number"
              className="w-full bg-primary border border-theme p-5 rounded-2xl text-cyan-400 font-black text-2xl outline-none focus:border-cyan-500 transition"
              value={baseBudget || ''}
          ...
          <div className="space-y-4 pt-6 border-t border-theme">
            ...
            <div className="h-32 overflow-y-auto bg-primary border border-theme rounded-xl p-4 leading-relaxed custom-scrollbar prose prose-invert max-w-none">
              <div dangerouslySetInnerHTML={{ __html: termsText || "<h3>Terms & Conditions</h3><p>Loading developer terms of service...</p>" }} />
            </div>
            <label className="flex items-start gap-3 cursor-pointer">
              <input
                ...
                className="mt-1 w-4 h-4 accent-cyan-500 bg-primary border border-theme rounded"
              />
